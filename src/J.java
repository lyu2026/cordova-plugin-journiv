...
public class J extends CordovaPlugin{
	...
	class D extends SQLiteOpenHelper{
		...
		void clear(S s)throws Exception{
			if(s!=null){Cursor c=getReadableDatabase().rawQuery("SELECT imgs,files FROM d",null);while(c.moveToNext()){delRemoteFiles(s,c.getString(0));delRemoteFiles(s,c.getString(1));}c.close();}
		...
		void updateLinks(long id,String imgs,String files){ContentValues v=new ContentValues();v.put("imgs",imgs);v.put("files",files);getWritableDatabase().update("d",v,"id=?",new String[]{String.valueOf(id)});}
		...
		JSONObject page(JSONObject q,int pg,int sz)throws Exception{
			...
			JSONArray a=new JSONArray();Cursor c=getReadableDatabase().rawQuery("SELECT * FROM d"+w.toString(),p.toArray(new String[0]));while(c.moveToNext())a.put(row(c));c.close();
			JSONObject r=new JSONObject();r.put("data",a);r.put("total",total);r.put("page",pg);r.put("size",sz);return r;
		}

		int count(String w,String[] a){Cursor c=getReadableDatabase().rawQuery("SELECT COUNT(*) FROM d"+w,a);c.moveToFirst();int n=c.getInt(0);c.close();return n;}

		JSONObject one(int id){Cursor c=getReadableDatabase().query("d",null,"id=?",new String[]{String.valueOf(id)},null,null,null);JSONObject o=c.moveToFirst()?row(c):new JSONObject();c.close();return o;}

		JSONArray multi(int[] ids){JSONArray a=new JSONArray();for(int id:ids){JSONObject o=one(id);if(o.has("id"))a.put(o);}return a;}

		JSONArray memory(){JSONArray a=new JSONArray();Calendar c=Calendar.getInstance();int y=c.get(Calendar.YEAR);for(int i=1;i<=10;i++){c.set(Calendar.YEAR,y-i);String d=new SimpleDateFormat("MM-dd",Locale.getDefault()).format(c.getTime());Cursor cu=getReadableDatabase().rawQuery("SELECT * FROM d WHERE at LIKE ? ORDER BY at DESC",new String[]{"%"+d+"%"});JSONArray r=new JSONArray();while(cu.moveToNext())r.put(row(cu));cu.close();if(r.length()>0)a.put(r);}return a;}
	}
}